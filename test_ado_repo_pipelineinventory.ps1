...
#
#
#  Failure behaviour: any project or repo that fails is logged to the error log
#  and skipped — the script always continues to the next item and runs to completion.
#
#  Key design guarantees:
#  - Pre-flight validation: checks Org, PAT and API reachability before doing any work
#  - All pagination uses while($true) + explicit break — never do..while($token)
#  - Continuation tokens normalised to $null on empty string at the API layer
#  - Empty-value pages always break regardless of token presence (guards infinite loops)
#  - Safety page cap of 1000 pages (covers 100k items per scope at 100/page)
#  - Console output throttled at repo level — only project-level progress shown
...
# Single Api probe call — validates org name, PAT validity and network access
...
# =============================================================================
foreach ($f in @($PipelineCsv, $ProjectCountCsv, $ErrorLog)) {
...
#
#  CONFIRMED via Postman: the build/definitions list endpoint does NOT return
#  the process object in any item — the field is completely absent regardless
#  of $expand. The only reliable source is GET /build/definitions/{id} which
#  always includes the full process object with type and yamlFilename.

#  Signal order against the full definition response (first match wins):
#    1. process.yamlFilename non-empty → YAML Pipeline
#    2. process.type = "2"            → YAML Pipeline
#    3. process.type = "1"            → Classic Build
#    4. process present, type absent  → Classic Build
#    5. no process object             → Classic Build
# =============================================================================
function Resolve-PipelineType {
    param ([object]$Def, [string]$ProjectName)

    $url    = "https://dev.azure.com/$script:Org/$([Uri]::EscapeDataString($ProjectName))/_apis/build/definitions/$($Def.id)?api-version=7.1-preview.7"
    Wait-PoliteDelay
    $detail = Invoke-ADOApi -Url $url

    if (-not $detail -or -not $detail.Body) {
        Write-ErrorLog "Could not fetch definition detail for '$($Def.name)' in '$ProjectName' — defaulting to Classic Build"
        return "Classic Build"
    }

    $d = $detail.Body

    # Signal 1: yamlFilename present → definitively YAML
    if ($d.process -and ![string]::IsNullOrWhiteSpace($d.process.yamlFilename)) {
        return "YAML Pipeline"
    }

    # Signals 2 & 3: process.type — stringified to avoid Int32/Int64 cast issues (PS5 vs PS Core)
    if ($null -ne $d.process -and $null -ne $d.process.type) {
        $t = "$($d.process.type)"
        if ($t -eq "2") { return "YAML Pipeline" }
        if ($t -eq "1") { return "Classic Build"  }
    }

    # Signals 4 & 5: process present with no type, or no process at all → Classic Build
    return "Classic Build"
}
# =============================================================================
#  BUILD PIPELINE FETCH — scoped to one repo
#  On failure: logs error, returns partial count, caller continues to next repo
# =============================================================================
function Get-BuildPipelinesForRepo {
    param ([string]$ProjectName, [string]$RepoId, [string]$RepoName)
    ...
    while ($true) {
        ...
        if ($page -gt $script:MaxPagesPerScope) {
            Write-ErrorLog "SAFETY CAP — build pipelines for '$RepoName' in '$ProjectName' hit $($script:MaxPagesPerScope) page limit"
            break
        }

        $url = "https://dev.azure.com/$script:Org/$([Uri]::EscapeDataString($ProjectName))/_apis/build/definitions" +
               "?repositoryId=$RepoId&repositoryType=TfsGit&`$top=100&api-version=7.1-preview.7"
        ...
        if (-not $result -or -not $result.Body) {
            Write-ErrorLog "SKIPPED build pipelines (page $page) for '$RepoName' in '$ProjectName' — API returned null. Continuing to next repo."
            break
        }
        ...
        foreach ($def in $defs) {
            ...
            Write-PipelineCsvRow -Row ([PSCustomObject]@{
                Project      = $ProjectName
                Repository   = $RepoName
                PipelineName = $def.name
                PipelineType = Resolve-PipelineType -Def $def -ProjectName $ProjectName
...
# =============================================================================
#  STEP 2 — Per project: repos → build pipelines per repo → release pipelines
#
#  Failure handling:
#  - Repo page failure      → log, mark project as partial, move to release pipelines
#  - Individual repo failure → log, skip that repo, continue to the next repo
...
foreach ($proj in $allProjects) {
    ...
    # ETA estimate — shown after first project completes
    $etaStr = ""
    if ($projIndex -gt 1) {
        $elapsed   = (Get-Date) - $startTime
        $avgSecs   = $elapsed.TotalSeconds / ($projIndex - 1)
        $remaining = ($totalProjects - $projIndex) * $avgSecs
        $etaStr    = " — ETA ~$([math]::Round($remaining / 60, 1))m"
    }

    Write-Host "[$projIndex/$totalProjects] $projName$etaStr" -ForegroundColor Cyan
    ...
    while ($true) {
        ...
        $projRepoCount += $pageCount
        $totalRepos    += $pageCount

        # Throttle console output — printing every repo name at 50k scale floods the terminal
        if ($projRepoCount % 100 -eq 0 -or $pageNum -eq 1) {
            Write-Host "  Repos: $projRepoCount (page $pageNum)" -ForegroundColor Gray
        }

        foreach ($repo in $repos) {
            $repoPipCount   = Get-BuildPipelinesForRepo -ProjectName $projName -RepoId $repo.id -RepoName $repo.name
            $projPipelines += $repoPipCount
            $totalPipelines += $repoPipCount
        }
    ...
    # ── 2b. Release pipelines — always attempted even if repo paging was partial ──
    ...
    Write-ProjectCountRow -Row ([PSCustomObject]@{
        Project       = $projName
        RepoCount     = $projRepoCount
        PipelineCount = $projPipelines
    })

    if ($projPartial) {
        $failedProjects++
        Write-Host "  Partial — Repos: $projRepoCount | Pipelines: $projPipelines (check error log)" -ForegroundColor Yellow
    } else {
        Write-Host "  Done    — Repos: $projRepoCount | Pipelines: $projPipelines" -ForegroundColor Green
    }
}
